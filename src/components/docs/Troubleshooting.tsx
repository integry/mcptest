...
const expressCorsExample = `import cors from 'cors';
...
const Troubleshooting: React.FC = () => {
  ...
  const highlightedExpressCorsExample = useMemo(
    () => hljs.highlight(expressCorsExample, { language: 'javascript' }).value,
    []
  );

  const copyExpressCorsExample = async () => {
    try {
      await navigator.clipboard.writeText(expressCorsExample);
      setCopied(true);
      ...
      <div className="row justify-content-center">
        <div className="col docs-page">
          <div className="docs-layout">
            <article className="docs-article troubleshooting-article" aria-labelledby="troubleshooting-title">
          <h1 id="troubleshooting-title" className="mb-4">Troubleshooting Remote MCP Servers</h1>
          ...
          <p>
            ...
            are fallbacks. A bare origin returning <code className="docs-code-error">404</code> therefore
            says little about a custom endpoint.
          </p>
          <p>
            ...
            to the forwarded port. A <code className="docs-code-error">401</code> or{' '}
            <code className="docs-code-error">403</code> proves the endpoint is reachable; it is an
            authentication outcome, not evidence that the server is stateful or broken.
          </p>
          ...
          <p>
            If discovery succeeds but the next request gets <code className="docs-code-error">400</code>,
            compare those fields character for character and inspect gateway logs. Proxies often drop unfamiliar headers unless they are
          ...
          <ul>
            <li><strong><code className="docs-code-error">400</code> after initialization:</strong> the client lost the session ID, CORS hid it, or the server issued it too late.</li>
            <li><strong>Intermittent <code className="docs-code-error">404</code>:</strong> session state is in one instance&apos;s memory and requests are reaching another, or the server expired the session.</li>
            ...
            <li><strong>DELETE or GET returns <code className="docs-code-error">405</code>:</strong> this can be valid because explicit termination and the standalone SSE stream are optional.</li>
          </ul>
          <p>
            A client should establish a new session after an expired-ID{' '}
            <code className="docs-code-error">404</code>. A server should use secure, unpredictable IDs
            and either shared storage or deliberate affinity when it scales out.
          </p>
          ...
          <div className="docs-code-block">
            <button
              ...
              onClick={copyExpressCorsExample}
              aria-label={copied ? 'Express CORS example copied' : 'Copy Express CORS example'}
              title={copied ? 'Copied' : 'Copy code'}
            >
              <i className={`bi ${copied ? 'bi-check-lg' : 'bi-copy'}`} aria-hidden="true"></i>
              <span aria-hidden="true">{copied ? 'Copied' : 'Copy'}</span>
            </button>
            <pre>
              <code
                className="hljs language-javascript"
                dangerouslySetInnerHTML={{ __html: highlightedExpressCorsExample }}
              />
            </pre>
          </div>
          <p>
          ...
          <p>
            ...
            tokens or API keys authenticate only the target MCP server. A{' '}
            <code className="docs-code-error">401</code> from the proxy and a{' '}
            <code className="docs-code-error">401</code> from the target have different remedies; inspect
            the response and message log before refreshing target credentials.
          </p>
          ...
          <div className="table-responsive">
            <table className="table">
              ...
              <tbody>
                <tr>
                  <td><code className="docs-code-error">400</code></td>
                ...
                <tr>
                  <td><code className="docs-code-error">401</code></td>
          ...
          <p>
            After a <code className="docs-code-error">401</code>, verify that{' '}
            <code>WWW-Authenticate</code> or the well-known URI leads to
          ...
          <p>
            <code className="docs-code-error">-32700</code> means invalid JSON,{' '}
            <code className="docs-code-error">-32601</code> means the method is absent, and{' '}
            <code className="docs-code-error">-32602</code> means invalid parameters. If a server advertises tools, resources, or
